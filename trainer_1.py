...
def train():
    ...
    deploy_to_huggingface(output_dir)

    # Verify sgLang compatibility
    prepare_sglang_compatible_model(output_dir)

def deploy_to_huggingface(output_dir):
    """Deploy the model to Hugging Face Hub"""
    api = HfApi()
    repo_name = "fine-tuned-mistral-vllm"
    username = HfFolder.get_token().split("_")[0]
    ...
    upload_folder(
        ...
        path_in_repo="",
        commit_message="Initial upload of fine-tuned model with vLLM support"
    )

    logger.info(f"Model deployed to Hugging Face Hub at {repo_url}")
    logger.info("Ensure vLLM compatibility is enabled on the Hugging Face model card.")

def prepare_sglang_compatible_model(output_dir):
    """Prepare the model for sgLang offline testing"""
    sglang_model_path = f"{output_dir}/sgLang_compatible_model"
    os.makedirs(sglang_model_path, exist_ok=True)

    # Copy necessary files
    for file_name in ["pytorch_model.bin", "config.json", "tokenizer.json", "special_tokens_map.json"]:
        src = os.path.join(output_dir, file_name)
        dst = os.path.join(sglang_model_path, file_name)
        if os.path.exists(src):
            os.rename(src, dst)

    logger.info(f"Model prepared for sgLang compatibility at {sglang_model_path}")
